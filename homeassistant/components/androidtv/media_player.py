...
REQUIREMENTS = ['androidtv==0.0.9', 'firetv==1.0.9']
...
DEVICE_CLASSES = [DEVICE_ANDROIDTV, DEVICE_FIRETV]

FRIENDLY_NAME_ANDROIDTV = 'Android TV'
FRIENDLY_NAME_FIRETV = 'Fire TV'
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    vol.Required(CONF_DEVICE_CLASS): vol.In(DEVICE_CLASSES),
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if config[CONF_DEVICE_CLASS] == DEVICE_ANDROIDTV:
        from androidtv import AndroidTV as ControllerClass
        DeviceClass = AndroidTVDevice
        device_kwargs = {'name': config[CONF_NAME],
                         'apps': config[CONF_APPS]}
        friendly_name = FRIENDLY_NAME_ANDROIDTV

    else:
        from firetv import FireTV as ControllerClass
        DeviceClass = FireTVDevice
        device_kwargs = {'name': config[CONF_NAME],
                         'apps': config[CONF_APPS],
                         'get_sources': config[CONF_GET_SOURCES]}
        friendly_name = FRIENDLY_NAME_FIRETV
    ...
    if CONF_ADB_SERVER_IP not in config:
        ...
        if CONF_ADBKEY in config:
            ...
            aftv = ControllerClass(host, config[CONF_ADBKEY])
            adb_log = " using adbkey='{0}'".format(config[CONF_ADBKEY])
        else:
            aftv = ControllerClass(host)
            adb_log = ""
    else:
        # Use "pure-python-adb" (communicate with ADB server)
        aftv = ControllerClass(host, adb_server_ip=config[CONF_ADB_SERVER_IP],
    ...
    if not aftv.available:
        _LOGGER.warning("Could not connect to %s at %s%s",
                        friendly_name, host, adb_log)
        return

    if host in hass.data[ANDROIDTV_DOMAIN]:
        _LOGGER.warning("Platform already setup on %s, skipping", host)
    else:
        device = DeviceClass(aftv, **device_kwargs)
        add_entities([device])
        _LOGGER.debug("Setup %s at %s%s", friendly_name, host, adb_log)
...
class ADBDevice(MediaPlayerDevice):
    ...
    def __init__(self, aftv, name, apps, keys):
...
class AndroidTVDevice(ADBDevice):
    ...
    def __init__(self, aftv, name, apps):
        ...
        super().__init__(aftv, name, APPS, KEYS)
