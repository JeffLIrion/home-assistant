...
def checksum_from_bytes(s):  # noqa: D103
...
def MAKE_CMD(type, cmd):  # noqa: D103
    return (type << 8) | cmd


class Packet:  # noqa: D101
    _CMD_TIMEOUT = 5
    ...
    def __init__(self, cmd, payload=bytes()):  # noqa: D107
        self._cmd = cmd
    ...
    def __str__(self):  # noqa: D105
        if self._cmd == self.ASYNC_ACK:
            ...
            return "Packet: Cmd={:04X}, Payload={}".format(
    ...
    @property
    def Cmd(self):  # noqa: D102
        return self._cmd

    @property
    def Payload(self):  # noqa: D102
        return self._payload

    def Send(self, fd):  # noqa: D102
        pkt = bytes()
...
class SensorEvent:  # noqa: D101
    ...
    def __str__(self):  # noqa: D105
        s = "[{}][{}]".format(self.Timestamp.strftime("%Y-%m-%d %H:%M:%S"), self.MAC)
        if self.Type == "state":
            s += (
                "StateEvent: sensor_type=%s, state=%s, battery=%d, signal=%d"
                % self.Data
            )
        else:
            s += "RawEvent: type={}, data={}".format(self.Type, bytes_to_hex(self.Data))
        return s


class Dongle:  # noqa: D101
    _CMD_TIMEOUT = 5

    class CmdContext:  # noqa: D106
    ...
    def __init__(self, device, event_handler):  # noqa: D107
    ...
    def Stop(self, timeout=_CMD_TIMEOUT):  # noqa: D102
        self.__exit_event.set()
        os.close(self.__fd)
        self.__fd = None
        self.__thread.join(timeout)

    def Scan(self, timeout=60):  # noqa: D102
...
def Open(device, event_handler):  # noqa: D103
